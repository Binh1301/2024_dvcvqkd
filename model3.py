...
class QKDParams:
    def __init__(self):
        ...
        self.L_OGS = 1.029           # km
        self.L_atm = 20.000            # km
        # Other
        self.p_thr = 1e-6  
        self.codeblock = 1e11          # Probability threshold for scintillation
...
class Sayat2024Channel:
    ...
    def get_geometry(self, H_sat, theta_deg):
        ...
        # Khoảng cách tổng
        sin_val1 = (np.cos(theta) * (RE_LOGS + self.p.L_OGS) / (6371 + H_sat))
        alpha1 = np.arcsin(sin_val1) + (np.pi/2 - theta)
        L_tot = np.sqrt((6371 + H_sat)**2 + (RE_LOGS +self.p.L_OGS)**2 - 
                        2*(6371 + H_sat)*(RE_LOGS + self.p.L_OGS)*np.cos(alpha1))

        # Khoảng cách khí quyển hiệu dụng
        sin_val2 = (np.cos(theta) * (RE_LOGS + self.p.L_OGS) / (6371000 + self.p.L_atm))
        alpha2 = np.arcsin(sin_val2) + (np.pi/2 - theta)
        L_atm_eff = np.sqrt((6371000 + self.p.L_atm)**2 + (RE_LOGS + self.p.L_OGS)**2 - 
                            2*(6371000 + self.p.L_atm)*(RE_LOGS + self.p.L_OGS)*np.cos(alpha2))
    ...
    # λ (nm)
        lambda_nm = self.p.wavelength * 1e9

    # Hệ số 4.343 = 10 log10(e)
        coeff = 4.343 * (3.912 / V_vis)

    # Scattering loss per km
        Ascat_per_km = coeff * (lambda_nm / 550)**(-p)

    # Tổng tổn hao
        Ascat = Ascat_per_km * L_atm_eff

        return Ascat
        
    def Cn2_HV(self, z, v=21, A=1e-14):
        return (0.00594 * (v/27)**2 * (1e-5*z)**10 * np.exp(-z/1000)
...
# ASCI CHUẨN
# ================================
    def get_asci(self, L_atm_eff, D_r):
        k = 2*np.pi / self.p.wavelength
        L_m = L_atm_eff * 1000  # km → m

    # ===== TÍCH PHÂN CHUẨN =====
        def integrand(z):
            return self.Cn2_HV(z) * (L_m - z)**(5/6)

        integral, _ = quad(integrand, 0, L_m)

        sigma_R2 = 2.25 * k**(7/6) * integral

    # ===== Aperture averaging =====
        d = D_r * np.sqrt(np.pi / (2 * self.p.wavelength * L_m))

        t1 = (0.20 * sigma_R2) / ((1 + 0.18*d**2 + 0.20*sigma_R2**(6/5))**(7/6))
        t2 = (0.21 * sigma_R2 * (1 + 0.24*sigma_R2**(6/5))**(-5/6)) / \
            (1 + 0.90*d**2 + 0.21*d**2*sigma_R2**(6/5))
    ...
    def compute_all(self, H_sat, theta_deg, V_vis, Cn2, D_r):
        ...
        Asci = self.get_asci(L_atm_eff, D_r)
