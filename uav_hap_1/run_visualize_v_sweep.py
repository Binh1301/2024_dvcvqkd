...
if __package__ in (None, ""):
    ...
    from uav_hap_1.visualization.v_sweep import SweepSeries, plot_nu_sweep
...
def main() -> None:
    ...
    # MB mode: "fixed" uses QAM_NU_TILDE, "tuned" matches VA to binomial
    mb_mode = "tuned"
    ...
    if mb_mode == "tuned":
        ...
        mb_label = f"mb (nu={nu_tuned:.3f}, VA~{va_tuned:.3f})"
    else:
    ...
    def sweep_one(state, label, sweep_name, sweep_values):
        ...
        return np.array(skr_vals, dtype=float), np.array(status_vals, dtype=object)

    # SKR vs T
    bin_skr_T, bin_status_T = sweep_one(bin_state, "binomial", "T", T_values)
    uni_skr_T, uni_status_T = sweep_one(uni_state, "uniform", "T", T_values)
    mb_skr_T, mb_status_T = sweep_one(mb_state_fixed, "mb", "T", T_values)

    _plot_skr_sweep(
        T_values,
        [
            {"label": "binomial", "skr": bin_skr_T, "status": bin_status_T},
            {"label": "uniform", "skr": uni_skr_T, "status": uni_status_T},
            {"label": mb_label, "skr": mb_skr_T, "status": mb_status_T},
        ],
        xlabel="T_eff",
    ...
    # SKR vs eps
    bin_skr_eps, bin_status_eps = sweep_one(bin_state, "binomial", "eps", eps_values)
    uni_skr_eps, uni_status_eps = sweep_one(uni_state, "uniform", "eps", eps_values)
    mb_skr_eps, mb_status_eps = sweep_one(mb_state_fixed, "mb", "eps", eps_values)

    _plot_skr_sweep(
        eps_values,
        [
            {"label": "binomial", "skr": bin_skr_eps, "status": bin_status_eps},
            {"label": "uniform", "skr": uni_skr_eps, "status": uni_status_eps},
            {"label": mb_label, "skr": mb_skr_eps, "status": mb_status_eps},
        ],
        xlabel="eps",
    ...
    # SKR vs v_el
    bin_skr_v, bin_status_v = sweep_one(bin_state, "binomial", "v_el", v_el_values)
    uni_skr_v, uni_status_v = sweep_one(uni_state, "uniform", "v_el", v_el_values)
    mb_skr_v, mb_status_v = sweep_one(mb_state_fixed, "mb", "v_el", v_el_values)

    _plot_skr_sweep(
        v_el_values,
        [
            {"label": "binomial", "skr": bin_skr_v, "status": bin_status_v},
            {"label": "uniform", "skr": uni_skr_v, "status": uni_status_v},
            {"label": mb_label, "skr": mb_skr_v, "status": mb_status_v},
        ],
        xlabel="v_el",
    ...
    nu_series = [
        SweepSeries(
            ...
            np.full_like(nu_values, bin_metrics_fixed.skr, dtype=float),
        ),
        SweepSeries(
            "uniform",
            nu_values,
            ...
            np.full_like(nu_values, uni_metrics_fixed.skr, dtype=float),
        ),
        SweepSeries("mb", nu_values, np.array(mb_z), np.array(mb_chi), np.array(mb_iab), np.array(mb_skr)),
    ]
    try:
    ...
    print(f"Saved plots: {out_dir / 'skr_vs_T.png'}, {out_dir / 'skr_vs_eps.png'}, {out_dir / 'skr_vs_v_el.png'}")
