...
def _eta_atm(channel_params: ChannelParams, L_m: float) -> tuple[float, float]:
    if channel_params.xi_per_km is None:
        xi_per_km = kruse_xi_per_km(
            visibility_km=float(channel_params.visibility_km),
            wavelength_m=float(channel_params.wavelength_m),
        )
    else:
        xi_per_km = float(channel_params.xi_per_km)
...
def _shape_parameters(a_m: float, W_L_m: float) -> dict:
    ...
    x = (2.0 * a / w_l) ** 2
    t0_amp = float(np.sqrt(np.clip(1.0 - np.exp(-2.0 * a**2 / w_l**2), 0.0, 1.0)))
    ...
    ratio = max((2.0 * t0_amp**2) / term, 1.0 + EPS)
    ...
    r_m = float(a / (ln_ratio ** (1.0 / gamma)))
...
def _sigma2_uav(channel_params: ChannelParams, a_m: float) -> float:
    ...
    return float(sigma2_pos + float(a_m) ** 2 * sigma2_orient)
...
def _sigma2_turb(
    ...
    if not channel_params.use_hv_turbulence:
        return float(
            1.919
            * float(channel_params.Cn2)
            * float(L_m) ** 3
            * (2.0 * max(float(W0_m), EPS)) ** (-1.0 / 3.0)
        )

    cos_zeta = max(float(np.cos(float(zeta_rad))), np.sqrt(EPS))

    def integrand(h_m: float) -> float:
        return Cn2_HV(h_m, w_wind=channel_params.w_wind, Cn2_0=channel_params.Cn2_0) * (
            h_m - float(geometry.H_UAV_m)
        ) ** 3

    integral, _ = quad(integrand, float(geometry.H_UAV_m), float(geometry.H_HAP_m), limit=200)
    return float(max(1.919 * (2.0 * W0_m) ** (-1.0 / 3.0) * integral / (cos_zeta**4), 0.0))
...
def channel(
    ...
    eta_atm, xi_per_km = _eta_atm(channel_params, L_m)
    ...
    sigma2_uav = _sigma2_uav(channel_params, a_m)
    sigma2_turb = _sigma2_turb(channel_params, geometry, L_m, zeta_rad, channel_params.W0_m)
    sigma2_r = float(max(sigma2_turb + sigma2_uav, 0.0))
    if channel_params.sigma_r_m is not None:
        sigma2_r = max(float(channel_params.sigma_r_m), 0.0) ** 2

    sigma_r = float(np.sqrt(sigma2_r))
    sigma_s = float(np.sqrt(max(sigma2_r / 2.0, 0.0)))

    n_samples = max(int(N), 1)
    if sigma_s > EPS:
        r_samples = np.asarray(generator.rayleigh(scale=sigma_s, size=n_samples), dtype=float)
    else:
        r_samples = np.zeros(n_samples, dtype=float)

    exponent = np.power(np.maximum(r_samples, 0.0) / shape["R_m"], shape["Gamma"])
    T_field_samples = shape["T0_amp"] * np.sqrt(np.exp(-exponent))
    eta_point_samples = np.clip(T_field_samples**2, 0.0, 1.0)

    T_samples = np.clip(eta_atm * eta_point_samples, 0.0, 1.0)
    mean_T2 = float(np.mean(eta_point_samples))
    T_eff = float(eta_atm * mean_T2)

    eta_sys = float(channel_params.eta_SMF * channel_params.T_T * channel_params.T_R)
    ...
    return {
        ...
        "eta_point_samples": eta_point_samples,
        "T_samples": T_samples,
        "T_samples_with_optics": np.clip(T_samples * eta_sys, 0.0, 1.0),
        "mean_T2": mean_T2,
        "T_eff": T_eff,
