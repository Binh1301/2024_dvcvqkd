...
def run(config: dict[str, Any], *, config_path: Path, output_path: Path,
    ...
    independently_reported_qam_centers = {
        "deformed_full_logva_bad": 0.0028899078522808853,
        "deformed_full_logva_medium": 0.01222509015497053,
        "deformed_full_logva_good": 0.01629776752815814,
        "binomial_high_ps_bad": 0.10452738018102539,
        "binomial_high_gs_bad": 0.07027708466852345,
        "binomial_high_logva_negative_bad": 0.013647079930233303,
    }
    native_bad = bisections["deformed_full_logva_bad"]["center"]
    qam_bad = independently_reported_qam_centers["deformed_full_logva_bad"]
    independently_reported_single_thread_bad = 0.002889923634
    cross_run = {
        "fixture_sha256": "9c7cc47a202f6fbbd6f08db19cd3c09df4dd74cf8653d28388b88ebf8a070362",
        "same_canonical_fixture_and_path_confirmed": True,
        "native_centers": {name: row["center"] for name, row in bisections.items()},
        "independently_reported_qam_centers": independently_reported_qam_centers,
        "independently_reported_single_thread_bad_center": independently_reported_single_thread_bad,
        "bad_boundary_reproducibility_interval": [
            min(native_bad, qam_bad, independently_reported_single_thread_bad),
            max(native_bad, qam_bad, independently_reported_single_thread_bad),
        ],
        "bad_boundary_cross_run_width": (
            max(native_bad, qam_bad, independently_reported_single_thread_bad)
            - min(native_bad, qam_bad, independently_reported_single_thread_bad)
        ),
        "transition_eigenvalue_scale_of_disagreement": "order 1e-18 around threshold 1e-13",
        "native_outward_accepted_counts": [row["accepted"] for row in rollback],
        "independently_reported_qam_outward_accepted_counts": [0, 4, 49],
        "interpretation": (
            "the nominal 1e-12 bisection width is not cross-run reproducible at this "
            "eigenvalue gap; use a numerical guard band rather than an exact scalar boundary"
        ),
    }
    dependencies = {
        "config": config_path,
    ...
    result = {
        ...
        "interpretation": (
            "naive exact-mask rollback can trap outward motion near a boundary; any proposed "
            "protocol needs an enhanced segment guard/backtracking rule and prospective review"
        ),
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "results" / "support_boundary_bisection_crn.json")
    parser.add_argument("--skip-crn", action="store_true")
    args = parser.parse_args()
    result = run(
        load_yaml(args.config), config_path=args.config.resolve(),
        output_path=args.output.resolve(), skip_crn=args.skip_crn,
    )
