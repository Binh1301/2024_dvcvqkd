...
MANIFEST = ROOT / "configs/manifold_consistent_synthetic_vjp_validation_execution_manifest_v1.json"
OUTPUT = ROOT / "results/manifold_consistent_synthetic_vjp_validation_v1.json"
...
def _scalar_directional(function, variable: torch.Tensor, direction: torch.Tensor, h: float) -> dict[str, Any]:
    ...
    numerical = float((function(variable.detach() + h * direction) - function(variable.detach() - h * direction)) / (2 * h))
    return {"analytic": analytic, "numerical": numerical, "absolute_error": abs(analytic - numerical), "finite": bool(torch.isfinite(gradient).all()), "hermitian_gradient": False}


def _spectral_rows(h: float) -> list[dict[str, Any]]:
    matrix = torch.eye(64, dtype=torch.complex128, requires_grad=True) / 256
    direction = torch.diag(torch.linspace(-0.25, 0.25, 64, dtype=torch.float64)).to(torch.complex128)
    weight = torch.eye(64, dtype=torch.complex128)
    rows = []
    for name, function in (("inverse_sqrt", hermitian_inverse_sqrt), ("square_root", hermitian_sqrt)):
        scalar = lambda value: torch.sum((function(value).conj() * weight).real)
        values = _scalar_directional(scalar, matrix, direction, h)
        values["name"] = name
        values["hermitian_gradient"] = True
        rows.append(values)
    return rows
...
def _row_pass(row: dict[str, Any], tolerance: dict[str, Any]) -> bool:
    allowance = float(tolerance["absolute_tolerance"]) + float(tolerance["relative_tolerance"]) * max(abs(row["analytic"]), abs(row["numerical"]))
    row["allowance"] = allowance
    row["passes"] = row["finite"] and row["absolute_error"] <= allowance
    return row["passes"]
