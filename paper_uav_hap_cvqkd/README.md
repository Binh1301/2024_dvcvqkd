...
`docs/PAPER_CODE_ALIGNMENT.md` records which target pieces remain outside the
current source.
...
- The active state is `(T, epsilon_base)`; `epsilon_total` is derived after the action and is passed unchanged to MI and Holevo. The production phase coefficient remains unresolved.
- The active channel sampler composes atmospheric loss, normalized lognormal scintillation, generalized Rician pointing, and optional AoA outage with raw/physical-T diagnostics; production profile, aperture, AoA, and phase mappings remain unresolved and fail closed.
...
- Numerical corrections are reported through diagnostics; materially invalid density/covariance states raise errors. Holevo now evaluates the physical Z intersection with deterministic bounded maximization, but target numerical evidence remains to be rebound.
...
The active sampler now composes atmospheric loss, normalized lognormal
scintillation, generalized Rician pointing, and optional hard AoA outage,
followed by raw-to-physical transmittance admission with no clipping or floor.
The legacy constant-C_n^2 beam-wander equation is not used by that path.
Production runs remain blocked until the altitude profile, aperture-averaging,
turbulence-AoA, and common-turbulence phase mappings are author/source frozen.
