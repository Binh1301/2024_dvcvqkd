"""
Compare binomial vs uniform QAM-256 at the same channel T_eff.
...
if __package__ in (None, ""):
    ...
    from uav_hap_1_sample.protocol.qam_protocol import build_state_binomial, build_state_uniform, compute_metrics
else:
    ...
    from .protocol.qam_protocol import build_state_binomial, build_state_uniform, compute_metrics


def _print_result(label, state, metrics):
    print(f"{label:<10} | alpha0={state.alpha0:.6f}, Ncut={state.ncut}")
...
def main() -> None:
    ...
    print("=" * 70)
    print("Binomial vs Uniform (QAM-256)")
    print("=" * 70)
    print(f"T_eff = {T_eff:.6f}, eps = {QAM_EPS}, beta = {QAM_BETA}, eta = {QAM_ETA}, v_el = {QAM_V_EL}")
    print("-" * 70)
